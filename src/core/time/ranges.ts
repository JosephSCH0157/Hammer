...
export const normalizeCuts = (
  ...
  const normalized = cuts
    ...
    .filter(
      (cut) =>
        Number.isFinite(cut.inMs) &&
        Number.isFinite(cut.outMs) &&
        cut.outMs > cut.inMs,
    )
...
export const computeKeptDurationMs = (
  durationMs: number,
  normalizedCuts: CutRangeMs[],
): number => {
  const maxDuration =
    Number.isFinite(durationMs) && durationMs > 0 ? durationMs : 0;
  const removed = normalizedCuts.reduce(
    (sum, cut) => sum + (cut.outMs - cut.inMs),
    0,
  );
